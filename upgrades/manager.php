...
require_once dirname(__FILE__) . '/version.220.php';
...
class TawkToUpgradeManager {
    public function __construct($dependencies, $options) {
        ...
        $this->upgrades = array(
            TawkToUpgradeVersion220::get_version() => TawkToUpgradeVersion220::class,
        );

        $this->version_var_name = $options['version_var_name'];
        $this->current_setting = $this->model_setting->getSetting('module_tawkto');
    ...
    public function start() {
        ...
        if (empty($this->prev_var)) {
            ...
            $this->model_setting->editSetting('module_tawkto', $this->current_setting);
        }
        ...
        if (version_compare($this->prev_ver, $this->curr_ver) < 0) {
            ...
            foreach ($this->upgrades as $upgrade_ver => $upgrade) {
                ...
                $this->model_setting->editSettingValue('module_tawkto', $this->version_var_name, $upgrade_ver);
            }
        }
